 ...
 * </ul>
 *
 * <p>历史向量（……_VECTOR……）为在某次实现上用 Tink 原语对固定密钥生成的密文/签名快照，用于防止
 * “加密侧与解密侧共享同一处错误”的对称性缺陷：对方角色代码在不使用 Tink 的前提下独立校验这些值。
 * 若算法线格式发生演进，按 {@code scripts/} 或仓库维护说明重新生成并同步替换。
 */
public final class InteropFixtures {
    ...
    // ---------------------------------------------------------------------
    // 历史向量（固定密钥 + 由本仓库 Tink 实现生成的输出快照；填充与校验方式见维护说明）。
    //
    // 以下密文/签名为一次性快照（本库加密原语每次输出不同）：在某次实现上用固定密钥对上述样本
    // 生成后固化为常量，由「对方角色」（纯 BouncyCastle，不使用 Tink）独立解密/验签验证。若算法
    // 线格式发生演进，重新生成并同步替换（生成方法：运行一个临时 main/test，打印 RAW 原语在固定
    // 密钥下的输出，见 interop/ 文档的“维护与重新生成”一节）。
    // ---------------------------------------------------------------------
