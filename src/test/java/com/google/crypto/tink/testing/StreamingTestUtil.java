...
public final class StreamingTestUtil {
    ...
    public static void testSkipWithStream(
        ...
        try {
            brokenPtStream.skip(2L * plaintextSize);
            brokenPtStream.read();
    ...
    public static void testEncryptDecryptString(StreamingAead ags) throws Exception {
        ...
        WritableByteChannel ctChannel = Channels.newChannel(bos);
        try (Writer writer =
                     Channels.newWriter(ags.newEncryptingChannel(ctChannel, associatedData), StandardCharsets.UTF_8)) {
        ...
        Reader reader =
                Channels.newReader(ags.newSeekableDecryptingChannel(ctBuffer, associatedData), StandardCharsets.UTF_8);
        for (int i = 0; i < repetitions; i++) {
        ...
        for (int start = 0; start <= plaintext.length; start += 1 + start / 2) {
            for (int length = 1; length <= plaintext.length; length += 1 + length / 2) {
                ...
                if (read == -1) {
                    ...
                    assertEquals("Incorrect truncation: ", start, plaintext.length);
                } else {
