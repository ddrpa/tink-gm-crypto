...
import cc.ddrpa.crypto.tink.streamingaead.Sm4GcmHkdfStreamingParameters.HashType;
 ...
 *
 * <p>Each ciphertext uses a new SM4-GCM key that is derived from the key derivation key, a
 * randomly
 ...
 */
@AccessesPartialKey
public final class Sm4GcmHkdfStreaming extends NonceBasedStreamingAead {
     ...
     * @param hkdfAlg               the JCE MAC algorithm name, e.g., HmacSha256, used for the HKDF
     *                              key derivation.
     ...
     *                              segment has size ciphertextSegmentSize - getHeaderLength() -
     *                              firstSegmentOffset
...
//    Validators.validateSm4KeySize(keySizeInBytes);
    ...
    @Override
    public Sm4GcmHkdfStreamDecrypter newStreamSegmentDecrypter() throws GeneralSecurityException {
        return new Sm4GcmHkdfStreamDecrypter();
    }
    ...
    /**
     * Returns the expected size of the ciphertext for a given plaintext. The returned value
     * includes the header and offset.
     */
     ...
     * IV for each segment. By enforcing that only the method encryptSegment can increment this
     * state, we can guarantee that the IV does not repeat.
     */
    class Sm4GcmHkdfStreamEncrypter implements StreamSegmentEncrypter {
        ...
        /**
         * Encrypts the next plaintext segment. This uses encryptedSegments as the segment number
         * for the encryption.
         */
        @Override
        ...
        /**
         * Encrypt a segment consisting of two parts. This method simplifies the case where one part
         * of the plaintext is buffered and the other part is passed in by the caller.
         */
        @Override
        public synchronized void encryptSegment(
     ...
     */
    class Sm4GcmHkdfStreamDecrypter implements StreamSegmentDecrypter {
