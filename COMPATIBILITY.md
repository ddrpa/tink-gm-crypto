...
结果判据：主代码与测试全部编译通过，且全部单元测试通过（当前套件 331 个用例，含跨系统互操作
自检 `cc.ddrpa.interop.*Test`，见 [interop/](interop/README.md)）。

## 兼容矩阵

| Tink 版本 | 制品版本 | 构建与测试 | 最近验证 | 说明 |
| --- | --- | --- | --- | --- |
| 1.23.0 | 1.23.0.0 | ✅ 编译通过，331/331 | 2026-09-07 | 默认目标版本（`tink.version`=1.23.0）；使用公开序列化 API；新增 SM2 签名/标准 SM2 加密/SM2-KEM+SM4-GCM 混合，以及 Tink⇄纯 BC 的跨系统互操作自检 |
| 1.22.0 | 1.23.0.0 | ✅ 编译通过，331/331 | 2026-09-07 | 当前源码的支持下限；SM2 各子模块与互操作自检同样基于 ≥1.22 公开序列化 API |
